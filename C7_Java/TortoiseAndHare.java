...
public class TortoiseAndHare {
    public void hareMove(String animal) {
        ...
        switch (plod){
            case 1->{for (int i = 0; i <9 ; i++) {hareTrack.add(animal);}}
            case 2->{for (int i = 0; i <12 ; i++) {hareTrack.remove(animal);}}
            case 3->{for (int i = 0; i <1 ; i++) {hareTrack.add(animal);}}
            case 4->{for (int i = 0; i <2 ; i++) {hareTrack.remove(animal);}}
        }
    }
    private final List<String> tortoiseTrack = new ArrayList<>();
    private final List<String> hareTrack = new ArrayList<>();
    public int getTortoiseMove() {return tortoiseTrack.size();}
    public int getHareMove(){return hareTrack.size();}
    public void tortoiseMove(String animal) {
        ...
        if(randNum>=8 ) plod = 3;
        switch (plod){
            case 1->{for (int i = 0; i <3 ; i++) {tortoiseTrack.add(animal);}}
            case 2->{for (int i = 0; i <6 ; i++) {tortoiseTrack.remove(animal);}}
            case 3->{for (int i = 0; i <1 ; i++) {tortoiseTrack.add(animal);}}
        }
    }
    void race() {
        hareMove("H ");
